...
//  2018-05-25: OpenGL: Removed unnecessary backup/restore of GL_ELEMENT_ARRAY_BUFFER_BINDING since this is part of the m_VAO state.
//  2018-05-14: OpenGL: Making the call to glBindSampler() optional so 3.2 context won't fail if the function is a nullptr pointer.
//  2018-03-06: OpenGL: Added const char* glsl_version parameter to ImGui_ImplOpenGL3_Init() so user can override the GLSL version e.g. "#version 150".
//  2018-02-23: OpenGL: Create the m_VAO in the render function so the setup can more easily be used with multiple shared GL context.
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
...
#ifndef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
    // This is part of m_VAO on OpenGL 3.0+ and OpenGL ES 3.0+.
    ...
    // Recreate the m_VAO every time (this is to easily allow multiple GL contexts to be rendered to. m_VAO are not shared among GL contexts)
    // The renderer would actually work without any m_VAO bound, but then our VertexAttrib calls would overwrite the default one currently bound.
